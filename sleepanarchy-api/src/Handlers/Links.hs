...
import App (DB (..), DBThrows (..), ThrowsError (..))
import Models.DB
    ...
    toSamples _ =
        singleSample
            LinkCategoryMap
                ...
                , lcmLinks =
                    [ LinkDetails
                        { ldTitle = "Child Link 1"
                        , ldSlug = "xUd2"
                        , ldDescription = "Some descriptive text"
                        , ldViews = 42
                        }
                    , LinkDetails
                        { ldTitle = "Page title or something better"
                        , ldSlug = "Iwn2"
...
getLinkCategory targetSlug = do
  ...
  where
    ...
    findCategory = mapM_ $ \lcMap ->
        if lcmSlug lcMap == targetSlug
