...
import Data.Time (UTCTime (..), addDays, fromGregorian, gregorianMonthLength)
...
mkPostData (Entity _ BlogPost {..}) (Entity _ category) =
    BlogPostListData
...
getBlogPost slug =
    runDB (getBy (UniqueBlogPost slug)) >>= \case
