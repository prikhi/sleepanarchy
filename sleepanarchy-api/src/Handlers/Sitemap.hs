...
import Web.Sitemap.Gen (ChangeFrequency (..), Sitemap (..), SitemapUrl (..))
...
generateSitemap = runDB $ do
    ...
    let newestPostDate =
        ...
        (postUrls, categories, archiveDates, tags) =
            foldr
  ...
  where
    ...
    mkCategoryUrl updatedMap (Entity catId BlogCategory {..}) =
        SitemapUrl
    ...
    mkArchiveUrl ((year, month), updated) =
        SitemapUrl
    ...
    mkLinkCategoryUrl updatedMap (Entity catId LinkCategory {..}) =
        SitemapUrl
