...
import Servant (Headers (..), ServerError (..), getHeaders)
    ...
    testGroup
        ...
        , testCase "Returns tags with counts alphabetically" $ do
            result <- testRunner $ runDB $ do
                ...
                P.insert_
                    bp
                        { blogPostTags = "clydefrog, artmeis"
                        , blogPostPublishedAt = Just $ blogPostCreatedAt bp
                        }
                P.insert_
                    bp
